...
def plt_intuition(x_train, y_train):
    ...
    @interact(w=(0, 400, 50), continuous_update=False)
    def func(w=150):
...
def plt_stationary(x_train, y_train):
    ...
    ### plot contour ###
    CS = ax[1].contour(tmp_w, tmp_b, np.log(z), levels=12, linewidths=2, alpha=0.7, colors=dlcolors)  # level控制有多少个等高圆圈
    ...
    ax[1].text(0.5, 0.95, "Click to choose w,b",
               ...
               fontsize=10,
               transform=ax[1].transAxes,  # 按整张图的位置来定，而不是按坐标轴的数值来定
               horizontalalignment='center'
               )

    # Surface plot of the cost function J(w,b)
    ax[2].plot_surface(tmp_w, tmp_b, z, cmap=dlcm, alpha=0.3, antialiased=True)  # Whether each patch in the collection should be drawn with antialiasing.
    ax[2].plot_wireframe(tmp_w, tmp_b, z, color='k', alpha=0.1)  # Plot a 3D wireframe.  'k' as black
    plt.xlabel("$w$")
    plt.ylabel("$b$")
    ax[2].zaxis.set_rotate_label(False)
    ax[2].xaxis.set_pane_color((1.0, 1.0, 1.0, 0.0))  # RGBA
